...
function getResendInstance() {
  ...
  if (!apiKey) {
    ...
    // En un entorno de servidor, esto debería ser un error fatal.
...
export async function sendAppointmentEmail(data: AppointmentData) {
  ...
  try {
      ...
      <body>
        <div class="container">
          ...
          <div class="section">
            ...
            <div class="info-row"><span class="info-label">Nombre:</span><span class="info-value">${data.name}</span></div>
            ...
            <div style="margin-top: 15px;"><div class="info-label">Dirección completa:</div><div style="margin-top: 5px; padding: 15px; background: #f8fafc; border-radius: 6px; border: 1px solid #e2e8f0;">${data.address}</div></div>
          </div>
          <div class="section">
          ...
          <div class="contact-buttons">
            ...
            <a href="https://wa.me/52${data.phone.replace(/\D/g, "")}" class="btn btn-whatsapp">💬 Abrir WhatsApp</a>
            <a href="tel:${data.phone}" class="btn btn-phone">📞 Llamar Ahora</a>
          </div>
          <div class="checklist">
          ...
          <div class="footer">
            ...
            <div style="font-size: 12px; opacity: 0.7; margin-top: 15px;">Generado automáticamente • ${new Date().toLocaleString("es-MX", { timeZone: "America/Mexico_City" })}</div>
          </div>
        </div>
      </body>
      </html>
    ...
    console.log("✅ Email admin enviado exitosamente! ID:", result.data?.id)
    return {
      success: true,
      provider: "resend",
...
export async function sendClientConfirmation(data: AppointmentData) {
  ...
  try {
    ...
    const clientHtmlContent = `
      ...
      <body>
        <div class="container">
          <div class="header"><h1>✅ Cita Recibida</h1><p style="margin: 8px 0 0 0; opacity: 0.9; font-size: 16px;">Nexu - Refacciones Pro</p></div>
          <div class="section">
            <div class="success-box"><h3 style="color: #166534; margin: 0 0 10px 0;">🎉 ¡Hola ${data.name}!</h3><p style="color: #166534; margin: 0;">Hemos recibido tu solicitud de cita para tu ${data.appliance}. Un técnico certificado se pondrá en contacto contigo en las próximas 2 horas para confirmar la fecha y horario.</p></div>
          ...
          <div class="section">
            ...
            <div class="info-box"><ul style="color: #1e40af; margin: 0; padding-left: 20px;"><li><strong>En 2 horas:</strong> Un técnico te contactará para confirmar</li><li><strong>24 horas antes:</strong> Te enviaremos un recordatorio</li><li><strong>El día de la cita:</strong> Diagnóstico gratuito y cotización</li><li><strong>Después del servicio:</strong> 30 días de garantía incluida</li></ul></div>
          </div>
          <div class="section">
            <div class="contact-info"><h3 style="color: #1e293b; margin: 0 0 15px 0;">📱 ¿Necesitas Contactarnos?</h3><p style="margin: 0 0 15px 0; color: #64748b;">Si tienes alguna pregunta o necesitas hacer cambios a tu cita:</p><p style="margin: 0;"><strong>WhatsApp:</strong> (33) 3876-6231<br><strong>Email:</strong> info@nexu.mx</p></div>
          </div>
          <div class="footer"><p style="margin: 0; font-size: 14px; opacity: 0.8;">Tu ID de Cita:</p><div class="appointment-id">${data.appointmentId}</div><p style="font-size: 12px; opacity: 0.7; margin: 15px 0 0 0;">Guarda este ID para futuras referencias</p></div>
        </div>
      </body>
      </html>
    `
    ...
    console.log("✅ Confirmación enviada al cliente! ID:", result.data?.id)
    return {
      success: true,
      provider: "resend",
      emailId: result.data?.id,
...
export async function testResendConfiguration() {
  try {
    ...
    const result = await resendInstance.emails.send({
      ...
      html: `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;"><h1 style="color: #10b981;">🎉 ¡Resend Configurado Correctamente!</h1><p>El sistema de emails de Nexu está funcionando perfectamente.</p><p>Ahora recibirás automáticamente todas las citas agendadas.</p><div style="background: #f0f9ff; padding: 15px; border-radius: 8px; margin: 20px 0;"><h3 style="color: #1e40af; margin: 0 0 10px 0;">Detalles del Test:</h3><p style="margin: 0;"><strong>Fecha:</strong> ${new Date().toLocaleString("es-MX")}</p><p style="margin: 0;"><strong>Estado:</strong> ✅ Funcionando</p><p style="margin: 0;"><strong>Proveedor:</strong> Resend</p></div><hr style="margin: 20px 0;"><small style="color: #64748b;">Test enviado automáticamente desde Nexu</small></div>`,
    })

    console.log("✅ Test enviado exitosamente! ID:", result.data?.id)
    return { success: true, emailId: result.data?.id, message: "Configuración exitosa. Revisa tu email." }
  } catch (error) {
